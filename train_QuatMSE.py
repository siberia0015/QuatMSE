...
from models.QuatMSE import QuatMSE
import json
import os


from argparse import ArgumentParser
parser = ArgumentParser("QuatMSE")
...
parser.add_argument("--tip", default='', help="备注")
args = parser.parse_args()

if args.model_name is None or len(args.model_name.strip()) == 0:
    args.model_name = "{}_lda-{}_nneg-{}_hs-{}_lr-{}_nepochs-{}_tip{}".format(args.dataset,
                                                                        args.lmbda,
                                                                        args.neg_num,
                                                                        args.hidden_size,
                                                                        args.learning_rate,
...
if args.mode == "train":
    print("mode: train")
    con.set_train_model(QuatMSE)
    con.training_model()
# prediction mode
else:
    print("mode: test")
    con.set_test_model(QuatMSE, args.checkpoint_path)
    con.test()
